## Summary

<!-- What changed and why? -->

## Related issue

<!-- Required: add a closing keyword so automation can validate the linked issue. -->
Closes #

## Type of change

<!-- Maintainers enforce at least one type:* label on every PR. If you have write access, add it when opening the PR. -->

- [ ] Bug fix (`type:bug`)
- [ ] Feature (`type:feature`)
- [ ] Documentation (`type:docs`)
- [ ] Chore / maintenance (`type:chore`)
- [ ] Security (`type:security`)

## Scope

<!-- Call out anything intentionally left out so review stays focused. -->

## Checklist

- [ ] Scope is focused and does not include unrelated changes
- [ ] PR body includes a linked closing issue reference
- [ ] A maintainer-applied or author-applied `type:*` label is present on the PR
- [ ] Tests were added or updated when behavior changed
- [ ] Documentation was updated when needed
- [ ] CHANGELOG.md was updated for user-facing changes
- [ ] I verified the affected area before requesting review
